//
//  MBV526ModelDetailViewController.swift
//  MiniCPM-V-demo
//
//  MiniCPM5-2B 纯文本模型详情页面 VC（下载管理 + 使用该模型）
//
...
@objc public class MBV526ModelDetailViewController: UIViewController, UIGestureRecognizerDelegate {
    
    var modelName: String = "MiniCPM5-2B"
    
    private let downloadManager = MBV526ModelDownloadManager.shared
    ...
    lazy var useModelButton: UIButton = {
        ...
        // title 由 updateUseModelButtonState 三态填写
    ...
    private func deleteAllDownloadedFiles() {
        ...
        let fileURL = documentsPath.appendingPathComponent(MiniCPMModelConst.modelv526_FileName)
    ...
    private func cleanupCacheFiles() {
        ...
        let fileName = MiniCPMModelConst.modelv526_FileName
    ...
    private func updateCellCompletion(modelName: String, success: Bool) {
        for (index, model) in dataArray.enumerated() where model.title == modelName {
            ...
            let hud = MBHUD.showAdded(to: self.view, animated: true)
            hud.mode = .text
            hud.label.text = String(format: (success
                                              ? L.ModelDetail.toastDownloadSuccessFormat
                                              : L.ModelDetail.toastDownloadFailedFormat).loc,
                                     modelName)
...
extension MBV526ModelDetailViewController: UITableViewDataSource {
...
extension MBV526ModelDetailViewController: UITableViewDelegate {
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        tableView.deselectRow(at: indexPath, animated: true)
        let model = dataArray[indexPath.row]
        guard let title = model.title else { return }
        
        if title == MiniCPMModelConst.modelv526_DisplayedName {
            handleModelDownload(modelName: title) { [weak self] in
                self?.downloadManager.downloadModelv526()
            }
        }
    }
    
    private func handleModelDownload(modelName: String, downloadAction: @escaping () -> Void) {
        if downloadManager.getModelv526_Status() == "downloaded" {
...
// MARK: - 数据配置 / 使用模型
extension MBV526ModelDetailViewController {
    
    public func loadTableViewData() {
        dataArray.removeAll()
        
        let languageModel = MBSettingsModel()
        languageModel.title = MiniCPMModelConst.modelv526_DisplayedName
        languageModel.icon = UIImage(systemName: "cpu")
        languageModel.statusString = getInitialStatus(for: downloadManager.getModelv526_Status())
    ...
    private func checkAllModelsDownloaded() -> Bool {
        ...
        return downloadManager.getModelv526_Status() == "downloaded"
    }
    ...
    fileprivate func updateUseModelButtonState() {
        ...
        case .needsDownload:
            ...
            useModelButton.setTitle(L.ModelDetail.mainButtonOneTapDownloadV526.loc, for: .normal)
    ...
    private func setAsCurrentModel() {
        ...
        UserDefaults.standard.setValue("V526TextModel", forKey: "current_selected_model")
