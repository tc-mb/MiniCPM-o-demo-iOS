//
//  MBV5ModelDetailViewController.swift
//  MiniCPM-V-demo
//
//  MiniCPM 5 纯文本模型详情页面 VC（下载管理 + 使用该模型）
//
...
@objc public class MBV5ModelDetailViewController: UIViewController, UIGestureRecognizerDelegate {
    ...
    private func deleteAllDownloadedFiles() {
        ...
        let fileURL = documentsPath.appendingPathComponent(MiniCPMModelConst.modelv5_FileName)
    ...
    private func cleanupCacheFiles() {
        ...
        let fileName = MiniCPMModelConst.modelv5_FileName
...
extension MBV5ModelDetailViewController: UITableViewDataSource {
...
extension MBV5ModelDetailViewController: UITableViewDelegate {
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        tableView.deselectRow(at: indexPath, animated: true)
        let model = dataArray[indexPath.row]
        guard let title = model.title else { return }
        
        if title == MiniCPMModelConst.modelv5_DisplayedName {
            handleModelDownload(modelName: title) { [weak self] in
                self?.downloadManager.downloadModelv5()
            }
        }
    }
    
    private func handleModelDownload(modelName: String, downloadAction: @escaping () -> Void) {
        if downloadManager.getModelv5_Status() == "downloaded" {
...
// MARK: - 数据配置 / 使用模型
extension MBV5ModelDetailViewController {
    
    public func loadTableViewData() {
        dataArray.removeAll()
        
        let languageModel = MBSettingsModel()
        languageModel.title = MiniCPMModelConst.modelv5_DisplayedName
        languageModel.icon = UIImage(systemName: "cpu")
        languageModel.statusString = getInitialStatus(for: downloadManager.getModelv5_Status())
    ...
    private func checkAllModelsDownloaded() -> Bool {
        ...
        return downloadManager.getModelv5_Status() == "downloaded"
    }
    ...
    fileprivate func updateUseModelButtonState() {
        ...
        case .needsDownload:
            ...
            useModelButton.setTitle(L.ModelDetail.mainButtonOneTapDownloadV5.loc, for: .normal)
    ...
    private func setAsCurrentModel() {
        ...
        UserDefaults.standard.setValue("V5TextModel", forKey: "current_selected_model")
