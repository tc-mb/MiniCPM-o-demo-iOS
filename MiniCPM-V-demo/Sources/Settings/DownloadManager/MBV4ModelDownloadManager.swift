            ...
            DispatchQueue.main.async {
                if progress >= 1 {
                    ...
                    if status == "failed" {
                        ...
                        self.progressHandler?(MiniCPMModelConst.modelv4_Q4_K_M_DisplayedName + L.Download.progressFailedSuffix.loc, -1)
        ...
        mmprojv4_Manager?.downloadV2(completionBlock: { [weak self] status, progress in
            ...
            DispatchQueue.main.async {
                if progress >= 1 {
                    ...
                    if status == "failed" {
                        self.setDownloadStatus(.failed, for: modelKey)
                        self.progressHandler?(MiniCPMModelConst.modelMMProjv4_DisplayedName + L.Download.progressFailedSuffix.loc, -1)
        ...
        mlmodelcv4_Manager?.downloadV2(completionBlock: { [weak self] status, progress in
            ...
            DispatchQueue.main.async {
                if progress >= 1 {
                    ...
                    if status == "failed" {
                        self.setDownloadStatus(.failed, for: modelKey)
                        self.progressHandler?(MiniCPMModelConst.mlmodelcv4_ZipFileDisplayedName + L.Download.progressFailedSuffix.loc, -1)
            ...
            DispatchQueue.main.async {
                ...
                if let checksum = checksum {
                    ...
                    if checksum == MiniCPMModelConst.modelv4_Q4_K_M_MD5 {
                        ...
                        MBModelArtifactStore.markValidated(fileURL, expectedMD5: MiniCPMModelConst.modelv4_Q4_K_M_MD5)
            ...
            DispatchQueue.main.async {
                ...
                if let checksum = checksum {
                    ...
                    if checksum == MiniCPMModelConst.modelMMProjv4_MD5 {
                        ...
                        MBModelArtifactStore.markValidated(fileURL, expectedMD5: MiniCPMModelConst.modelMMProjv4_MD5)
